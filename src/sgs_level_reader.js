...
const run = async () => {
    ...
    const input = connector.getInput('MySubscriber::LevelReader')
    try {
        ...
        for (let i = 0; i < 500; i++) {
            ...
            for (const sample of input.samples.validDataIter) {
                ...
                console.log('Received x: ' + data.sensor_id + ', ' + data.level + ', ' + data.capture_time)
                const res = await pool.query(
                    "INSERT INTO sgs.level (time, sensor_id, level) VALUES (to_timestamp($1), $2, $3)",
                    [data.capture_time, data.sensor_id, data.level]
                );
            }
        }
    } catch (err) {
