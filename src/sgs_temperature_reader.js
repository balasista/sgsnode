...
const run = async () => {
    ...
    const input = connector.getInput('MySubscriber::TemperatureReader')
    try {
        ...
        for (let i = 0; i < 500; i++) {
            ...
            for (const sample of input.samples.validDataIter) {
                ...
                console.log('Received x: ' + data.sensor_id + ', ' + data.temperature)
                const res = await pool.query(
                    "INSERT INTO sgs.temperature (time, sensor_id, temperature) VALUES (to_timestamp($1), $2, $3)",
                    [data.capture_time, data.sensor_id, data.temperature]
                );
            }
        }
    } catch (err) {
